...
def Test03(s,testres):
	...
	######################################################
	test=0;
	tester = App.CreateTester("01 Not 16")	
	for x in xrange(5000, 5100):
		tester.SetInput("in", x)
		tester.Evaluate()
		output=tester.GetOutput("out")
	...
	#######################################################	
	test=0;
	tester = App.CreateTester("02 Or8Way")
	for x in xrange(0, 2):
		for y in xrange(0, 2):
			for z in xrange(0, 2):
				tester.SetInput("x", x)
	...
	test=0;
	tester = App.CreateTester("03 ALU")
...
def Test05(s,testres):
	...
	##cycle##
	test=CPUtest(tester,test,0,0,1,-1,-1,-1)
	test=CPUtest(tester,test,12345,0,0,12345,-1,-1)
	test=CPUtest(tester,test,60432,0,0,12345,-1,-1)
	test=CPUtest(tester,test,23456,0,0,23456,-1,-1)
	test=CPUtest(tester,test,57808,0,0,23456,-1,-1)
	test=CPUtest(tester,test,1000,0,0,1000,-1,-1)
	test=CPUtest(tester,test,58120,0,0,1000,-1,-1)
	test=CPUtest(tester,test,1001,0,0,1001,-1,-1)
	test=CPUtest(tester,test,58264,0,0,1001,-1,-1)
	test=CPUtest(tester,test,1000,0,0,1001,-1,-1)
	test=CPUtest(tester,test,62672,65536,0,1001,-1,-1)
	test=CPUtest(tester,test,1001,65536,0,1001,-1,-1)
...
def CPUtest(tester,test,ins,inm,reset,adrs,pc,outm):
	tester = App.CreateTester("02 CPU")	
	...
	if (output1!=adrs and adrs!=-1):
		test=1	
	if (output2!=pc and pc!=-1):
		test=1
	if (output3!=outm and outm!=-1):
		test=1
		
	s+= "\n" + "addressM: ",output1," ,PC: ",output2," ,outM: ",output3," Test:" ,bool(1-test)," ,A register: ",output4," ,D register: ",output5
	return test
