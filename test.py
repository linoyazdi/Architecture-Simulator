...
def Test02(s,testres):
	...
	try:
		...
		s+= "\n" + "And04 Gate test ..." + str(sys.exc_info()[1])	
	#######################################################
	test=0;
	try:  
...
#######################################################
	...
	try: 
		...
		s+= "\n" + "Mux04 Gate test ..." + str(sys.exc_info()[1])
...
def Test03(s,testres):
	...
	try:  
		...
		s+= "\n" + "Not04 Gate test ..." + str(sys.exc_info()[1])	
	...
	try:  
		...
		for x in xrange(10101, 10102):
			for y in xrange(5003, 5004):
				for f in xrange(0, 2):
					for zx in xrange(0, 2):
						for zy in xrange(0, 2):
								...
								for ny in xrange(0, 2):
									for no in xrange(0, 2):
										...
										if (f==0):
											...
											temp=tx+ty
										if (temp>65536):
												#s+="\n"+str(temp)+" Larger than 2^16\n"
												temp=temp-65536
										if (no==1):
											temp=SNOT(temp)
											if (temp==(output*-1)):
												#s+="\n"+str(temp)+" equal to negative\n"
												temp=temp*-1
											elif (temp<0):
												temp=temp
												#s+="\n"+str(temp)+" smaller than 0\n"
										if (temp!=output):
											s+= "\n\npython:" + str(temp)+" output:"+str(output)+"\n nx:"+str(nx)+" ny:"+str(ny)+" zx:"+str(zx)+" zy:"+str(zy)+" no:"+str(no)+" x:"+str(x)+" tx:"+str(tx)+" y:"+str(y)+" ty:"+str(ty)+" f:"+str(f)
											#s+= "\n" + str(temp)+" "str(output)," nx ",nx,"ny ",ny," no ",no," x",x," tx ",tx," ",y," ty",ty	
											test=1
										ng=tester.GetOutput("ng")
										zr=tester.GetOutput("zr")
										tzr=0
										tng=0
										if (output<1):
											tzr=1
										...
										if (ng!=tng):
											test=1
											s+= "\n\npython:" + str(temp)+" output:"+str(output)+"\n nx:"+str(nx)+" ny:"+str(ny)+" zx:"+str(zx)+" zy:"+str(zy)+" no:"+str(no)+" x:"+str(x)+" tx:"+str(tx)+" y:"+str(y)+" ty:"+str(ty)+" f:"+str(f)
											s+= "\n\ng:" + str(ng)+" tng:"+str(tng)
											s+="\n NG error\n"
										if (zr!=tzr):
											test=1
											s+= "\n\npython:" + str(temp)+" output:"+str(output)+"\n nx:"+str(nx)+" ny:"+str(ny)+" zx:"+str(zx)+" zy:"+str(zy)+" no:"+str(no)+" x:"+str(x)+" tx:"+str(tx)+" y:"+str(y)+" ty:"+str(ty)+" f:"+str(f)					
											s+= "\n\zr:" + str(zr)+" tzr:"+str(tzr)
											s+="\n ZR error\n"
		if (test==0):
			s+= "\n" + "ALU Gate test ... Ok"
			total=total+1
		...
		s+= "\n" + "ALU Gate test ...Fail: " + str(sys.exc_info()[1])+"Fail\n"	
...
def CPUtest(tester,ins,inm,reset,adrs,pc,outm):
	...
	try:  
		...
		s+= "\n" + "|PC               : "+str(output2)
		s+= "\n" + "|---------------------------------"		
		s+= "\n" + "|addressM         : "+str(output1)
		s+= "\n" + "|---------------------------------"
		s+= "\n" + "|outM             : "+str(output3)
		s+= "\n" + "|---------------------------------"
		s+= "\n" + "|A register       : "+str(output4)
		s+= "\n" + "|---------------------------------"
		s+= "\n" + "|D register       : "+str(output5)
		s+= "\n" + "|---------------------------------"
		s+= "\n" + "|PC               : "+str(output2)
		s+= "\n" + "|---------------------------------"
		s+= "\n" + "|Expected address : "+str(adrs)
		s+= "\n" + "|---------------------------------"
		s+= "\n" + "|expexted output  : "+str(outm)
		s+= "\n" + "|---------------------------------"
		s+= "\n" + ""
		s+= "\n" + ""
